...
describe("transmuter (reverse)", () => {
  ...
  it("issues a full refund on reversal", async () => {
    ...
    expect(newBalance).to.be.gt(LAMPORTS_PER_SOL);
    expect(newBalance).to.be.lt(2 * LAMPORTS_PER_SOL);
    ...
    expect(newBalance2).to.be.gt(2 * LAMPORTS_PER_SOL);
    expect(newBalance2).to.be.lt(3 * LAMPORTS_PER_SOL);
  });
});
